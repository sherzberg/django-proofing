...
from proofing.models import PhotoSize, Photo

class Command(BaseCommand):
    help = ('Clears the Proofing cache for the given sizes.')
    args = '[sizes]'
...
def create_cache(sizes, options):
    ...
    if len(size_list) < 1:
        ...
        sizes = PhotoSize.objects.filter(slug__in=size_list)
    ...
    for photosize in sizes:
        print 'Flushing %s size images' % photosize.title
